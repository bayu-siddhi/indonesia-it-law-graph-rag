...
class RegulationParser:
    ...
    def __init__(self):
        ...
        self.REGULATION_ENCODING = encodings.REGULATION_CODES
        self.REGEX_PATTERNS = constants.PARSER_REGEX_PATTERNS
    ...
    def parse_regulations_content(
        ...
        for regulation_file in tqdm.tqdm(iterable=files, desc="Parsing regulations content", disable=not verbose):
                ...
                with open(filepath, "r", encoding="utf-8") as file:
                    ...
                    regulation_dict["content"]["consideration"] = {
                        "id": id_template.format(
                            reg_section=self.REGULATION_ENCODING["section"]["consideration"],
                            section_num="000",
                            extra_section_number="00"
                        ),
                        "text": re.search(self.REGEX_PATTERNS["main"]["consideration"], text, re.IGNORECASE)[1].strip()
                    }

                    # Extract "Mengingat" (observation) section
                    regulation_dict["content"]["observation"] = {
                        "id": id_template.format(
                            reg_section=self.REGULATION_ENCODING["section"]["observation"],
                            section_num="000",
                            extra_section_number="00"
                        ), 
                        "text": re.search(self.REGEX_PATTERNS["main"]["observation"], text, re.IGNORECASE)[1].strip()
                    }
        ...
        utils.list_of_dict_to_json(data=result, output_path=json_output)
        ...
        if verbose:
            ...
            print(f"{'Average time/file':<20}: {round(sum(durations) * 1000 / success, 3)} miliseconds")
            print("=" * 76)
        ...
        for article in articles:
            ...
            if article_number == "1":
                ...
                if re.search(self.REGEX_PATTERNS["article"]["check_definition"], article_text, re.IGNORECASE):
                    definitions = re.findall(self.REGEX_PATTERNS["article"]["definition"], article_text, re.IGNORECASE)
